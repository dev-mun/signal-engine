...
def performance_summary(equity_df: pd.DataFrame, trades_df: pd.DataFrame, initial_cash: float = 10_000.0) -> dict:
    ...
    if not completed_trades.empty and "PnL %" in completed_trades.columns:
        wins = completed_trades[completed_trades["PnL %"] > 0]
        losses = completed_trades[completed_trades["PnL %"] <= 0]

        win_rate = float((completed_trades["PnL %"] > 0).mean() * 100)
    ...
    return {
        ...
        "Completed Trades": int(len(completed_trades)),
        "Win Rate %": win_rate,
        "Average Win %": avg_win,
...
def save_reports(
    ...
    output_path = Path(output_dir) / "ema_rsi"
...
def print_scan_results(results: list[dict]) -> None:
    ...
    print("------------------")
    if not actionable:
        print("No actionable EMA RSI signals.")
    else:
...
def save_scan_results(results: list[dict], output_dir: str = "reports") -> None:
    ...
    print(f"\nSaved EMA RSI Watchlist Scan: {file_path}")
...
def plot_results(
    ...
    if not overlays["sell_markers"].empty:
        price_ax.scatter(
            overlays["sell_markers"].index,
            overlays["sell_markers"].values,
            label="SELL",
            color="#d62828",
            marker="v",
            s=90,
    ...
    price_ax.set_title(f"{label}: EMA RSI Pullback Strategy")
    ...
    rsi_ax.axhline(55, color="#adb5bd", linestyle="--", linewidth=1.0)
