...
def save_reports(
    ...
    output_path = Path(output_dir) / "four_hour"
...
def print_scan_results(results: list[dict]) -> None:
    actionable = [r for r in results if r["Signal"] in {"BUY", "SELL", "SHORT_SETUP", "SELL_SHORT", "COVER"}]
    errors = [r for r in results if r["Signal"] == "ERROR"]

    print("\nFour Hour Watchlist Scan")
    print("------------------------")
    print(
        f'{"Ticker":<8} '
        f'{"Strategy":<17} '
    ...
    for result in results:
        ...
        print(
            f'{result["Ticker"]:<8} '
            f'{result["Strategy"]:<17} '
            f'{result["Signal"]:<12} '
            f'{result["Setup"]:<15} '
            f'{result["Price"]:>10.2f} '
            f'{result["RSI"]:>8.2f} '
            f'{result["ATR"]:>8.2f}  '
            f'{result["Distance"]:<24} '
            f'{result["Reason"]}'
        )
    ...
    print("------------------")
    if not actionable:
        print("No actionable four-hour trend signals.")
    else:
...
def save_scan_results(results: list[dict], output_dir: str = "reports") -> None:
    ...
    print(f"\nSaved Four Hour Watchlist Scan: {file_path}")
...
def build_signal_overlay_data(
    ...
    return {
        ...
        "short_markers": close[signals["Signal"] == "SHORT_SETUP"],
...
def plot_results(
    ...
    if not overlays["short_markers"].empty:
        price_ax.scatter(overlays["short_markers"].index, overlays["short_markers"].values, label="SHORT_SETUP", color="#6d597a", marker="v", s=90, zorder=5)
    ...
    price_ax.set_title(f"{label}: 4H Trend Continuation Pullback")
    ...
    rsi_ax.axhline(45, color="#adb5bd", linestyle="--", linewidth=1.0)
