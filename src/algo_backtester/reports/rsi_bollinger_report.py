...
def save_reports(
    ...
    output_path = Path(output_dir) / "rsi_bollinger"
...
def print_scan_results(results: list[dict]) -> None:
    ...
    print("------------------")
    if not actionable:
        print("No actionable RSI Bollinger signals.")
    else:
...
def save_scan_results(results: list[dict], output_dir: str = "reports") -> None:
    ...
    print(f"\nSaved RSI Bollinger Watchlist Scan: {file_path}")
...
def plot_results(
    ...
    price_ax.set_title(f"{label}: RSI Bollinger Mean Reversion")
    ...
    rsi_ax.axhline(35, color="#adb5bd", linestyle="--", linewidth=1.0)
