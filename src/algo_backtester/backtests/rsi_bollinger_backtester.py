...
from algo_backtester.strategies.rsi_bollinger_mean_reversion import (
    add_indicators,
    classify_setup,
    distance_to_setup,
    should_buy,
...
class RsiBollingerBacktestConfig:
...
class RsiBollingerBacktester:
    def __init__(self, config: RsiBollingerBacktestConfig | None = None):
        self.config = config or RsiBollingerBacktestConfig()
    ...
    def run(self, raw_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        for i in range(1, len(df)):
            ...
            signal = "HOLD"
            reason = "No actionable RSI Bollinger setup."
...
def scan_ticker(
        ...
        config: RsiBollingerBacktestConfig | None = None,
) -> dict:
    raw_df = load_yfinance_data(ticker=ticker, start=start, end=end)
    bt = RsiBollingerBacktester(config=config)
